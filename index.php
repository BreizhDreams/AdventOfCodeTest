...
echo("Premier Résultat = ".$result);
echo("Second Résultat = ".$secondResult);
...
foreach ($data as $value){
    ...
    foreach ($data as $value2){
        if (similar_text($value,$value2) == strlen($value) -1){
            for ($i =0; $i < strlen($value);$i++){
                if($value[$i] == $value2[$i]){
                    $resultArray[]= $value[$i];
                }
            }
            $resultat = implode($resultArray);
        }
    }
}

echo("Résultat première Partie: ". $x * $y. "	");
echo("Résultat seconde Partie: ". $resultat);